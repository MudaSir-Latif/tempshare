...
def upload_file(request):
    if request.method == "POST":
        ...
        if not f:
            return render(request, "frontend/upload_result.html", {"error": "No file uploaded."})
        if f.size > MAX_UPLOAD:
            return render(request, "frontend/upload_result.html", {"error": "File too large (max 25MB)."})
...
def submit_url(request):
    if request.method == "POST":
        ...
        if not url:
            return render(request, "frontend/url_result.html", {"error": "No URL provided."})
